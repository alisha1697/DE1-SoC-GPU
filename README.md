...
## Running Unit Testbenches

Each module has a `run.do` script in its `tb/<module>/` folder. Run them from a ModelSim Tcl console using the full path — the scripts set their own working directory internally, so location doesn't matter:
...
For the full system test from repo root:
...
All scripts compile directly from `rtl/` — do not `cd` into `tb/<module>/` before running; the script handles paths itself.

---

## System Architecture

<!-- diagram: gpu_system_arch.svg|820 -->

`de1soc_top.sv` is the synthesis top. It wires board I/O (clock, buttons, LEDs, HEX displays) to `gpu_top`, and instantiates the two Quartus memory IPs (`matrix_ab`, `matrix_c`) outside `gpu_top`. Everything inside the dashed boundary is `gpu_top`.
...
`scheduler.sv` is the kernel FSM that runs inside each core. It orchestrates the inner product loop, issuing memory requests to the shared arbiters and stalling when they're not granted.

<!-- diagram: gpu_scheduler_fsm.svg|820 -->

**State summary:**

| State       | What happens                                                                                                                                                               |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `IDLE`      | Waiting for `start`. Resets k, t counters.                                                                                                                                 |
| `INIT`      | Asserts `kernel_init` for one cycle to reset thread accumulators.                                                                                                          |
| `WAIT_AB`   | Issues `a_req_valid` and `b_req_valid`. Stays here until both `a_have` and `b_have` are set (responses may arrive on different cycles). Pulses `fma_en[t]` once both land. |
| `NEXT_T`    | Advances `t`. If more threads remain this k-iteration → back to `WAIT_AB`. If last thread → `NEXT_K`.                                                                      |
| `NEXT_K`    | Advances `k`. If more k-iterations remain → back to `WAIT_AB` with `t=0`. If `k==N-1` → `WRITE_REQ`.                                                                       |
| `WRITE_REQ` | Issues `c_req_valid`. Stays here until `c_req_ready` (grant).                                                                                                              |
| `NEXT_W`    | Advances write-phase `t`. If more threads to write → back to `WRITE_REQ`. If last → `DONE_ST`.                                                                             |
| `DONE_ST`   | Holds until `rst` or next `start` (→ `INIT`).                                                                                                                              |

**Key signals:**

```systemverilog
a_have      // set when a_resp_valid lands; cleared at INIT/NEXT_T
b_have      // same for B
a_inflight  // req sent but resp not yet received (prevents re-issuing)
stall_cycles // increments every cycle at least one request is pending but not granted
```

The `done` output is combinational: `assign done = done_r && !start;` — this prevents a one-cycle overlap where `done` is still high on the same cycle `start` arrives (which caused the dispatcher to free the core mid-handshake in early builds).

---

### Core

`core.sv` wraps the scheduler, all thread instances, A/B response latches, and the address MUXes for the shared bus.

<!-- diagram: gpu_core_arch.svg|820 -->

**Why the latches exist:** the shared BRAM read port changes every cycle as the arbiter grants different cores. Once a core's read data arrives on `resp_valid`, it's only valid for one cycle before the bus moves on. The A/B latches (`a_latched`, `b_latched`) capture that data and hold it stable for the threads until `fma_en` fires.

**Why the address MUX exists:** all `THREADS_PER_CORE` threads generate their own `addr_A`, `addr_B`, `addr_C` each cycle. Only one thread is requesting the bus at a time (`t_select`), so the MUX picks that thread's address to drive the shared arbiter port.

**Dispatcher handshake:**

```systemverilog
core_valid[i] && core_ready[i]   // handshake fires
→ dispatcher pulses core_start[i] for one cycle
→ core.sv latches thread_id_start, thread_count
→ scheduler moves IDLE → INIT
```
---

### Round-Robin Arbiters

<!-- diagram: gpu_arbiter_protocol.svg|820 -->

Both `rr_read_arbiter.sv` and `rr_write_arbiter.sv` use the same round-robin scan: starting from a rotating priority pointer `ptr`, the first requester found wins that cycle. `ptr` advances past the winner so no single core monopolises the bus.

**Read arbiter** (`rr_read_arbiter.sv`): grant is combinational — `req_ready` goes high the same cycle as `req_valid` when the core wins arbitration, and the BRAM address is driven that cycle. `resp_valid` fires `BRAM_LATENCY` cycles later, implemented as a shift register (`grant_pipe[BRAM_LATENCY]`). The default is 1 (matches `dual_port_bram`); `de1soc_top` overrides to 2 for the real `matrix_ab` altsyncram IP.

**Write arbiter** (`rr_write_arbiter.sv`): writes commit the same cycle they're granted — no response path is needed. A losing core keeps `req_valid` high and re-requests next cycle.

**Stall counter:** `stall_cycles` increments any cycle where at least one core has `req_valid` high but did not receive `req_ready`. This is the primary contention metric — at `NUM_CORES=4` sharing one port you should consistently see non-zero counts.

### Thread

`thread.sv` computes one output element of matrix C. Each thread instance is assigned a unique `thread_id` (the flat index of the C element it owns). From that it derives:

```systemverilog
row = thread_id / N;
col = thread_id % N;
addr_A = base_addr_A + (row * N) + k;   // A[row][k]
addr_B = base_addr_B + (k  * N) + col;  // B[k][col]
addr_C = base_addr_C + (row * N) + col; // C[row][col]
```

The accumulator runs across all `k` iterations. On `kernel_init` (pulsed by the scheduler at the start of each block) the accumulator resets to 0, so a thread hardware instance reused across back-to-back block dispatches never carries stale partial sums. On the final `k = N-1` iteration, `result` is latched with the completed C element value ready for the write phase.

`data_ready` is permanently tied high — threads never back-pressure the memory system; stalling is the scheduler's job.

---

### FMA

`fma.sv` is a one-cycle pipelined fused multiply-accumulate: `result = (a * b) + c`. It takes a `valid_in` strobe and returns `valid_out` exactly one cycle later.

```systemverilog
assign product         = a * b;                      // 32-bit intermediate
assign product_aligned = product[DATA_WIDTH-1:0];    // truncate to 16 bit
// registered output:
if (valid_in) result <= product_aligned + c;
valid_out             <= valid_in;                   // propagates one cycle
```

`c` is the accumulator fed from `thread.sv`. The one-cycle latency means `thread.sv` must wait for `fma_valid_out` before updating `accumulator` — which it does, registering `fma_result` back into `accumulator` on the `fma_valid_out` edge.

**Note:** the current implementation truncates the 32-bit product to the lower 16 bits. For large `N` or large operand values this will overflow — a known limitation of the current fixed-function datapath.
